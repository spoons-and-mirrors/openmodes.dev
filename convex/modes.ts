...
export const listModes = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Get all modes to calculate total downloads per mode family
    const allModes = await ctx.db.query("modes").collect();

    return filteredModes.map((mode) => {
      // Get all versions of this mode to calculate total downloads and votes
      const allVersionsOfMode = allModes.filter(
        (m) => m.name === mode.name && m.status === "approved",
      );
      const totalDownloads = allVersionsOfMode.reduce(
        (sum, version) => sum + version.downloads,
        0,
      );
      const totalVotes = allVersionsOfMode.reduce(
        (sum, version) => sum + version.votes,
        0,
      );

      // Find the version with the most votes
      const topVersion =
        allVersionsOfMode.length > 0
          ? allVersionsOfMode.reduce((prev, current) =>
              current.votes > prev.votes ? current : prev,
            )
          : mode; // fallback to current mode if no approved versions found
      ...
      return {
        ...
        status: mode.status,
        topVersion: topVersion.votes === 0 ? "-" : topVersion.version,
      };
    });
  },
});
    ...
    const mode = await ctx.db
      ...
      .filter((q) =>
        q.and(
          ...
          q.eq(q.field("is_canonical"), true),
        ),
      )
      .first();
...
export const getModeByNameAndVersion = query({
  args: {
    name: v.string(),
    version: v.string(),
  },
  ...
  handler: async (ctx, args) => {
    // Use the same robust approach as getModeWithVersions - get all versions by name first
    ...
    const mode = allVersions.find((v) => v.version === args.version);
...
export const checkModeNameAvailability = query({
  args: { name: v.string() },
  returns: v.boolean(),
  handler: async (ctx, args) => {
    ...
    return !existingMode; // Returns true if name is available (no existing mode found)
  },
});

// Debug query to check what versions exist for a mode
export const debugModeVersions = query({
  args: { name: v.string() },
  returns: v.array(
    v.object({
      _id: v.id("modes"),
  ...
  handler: async (ctx, args) => {
    ...
    return modes.map((mode) => ({
      ...
      version: mode.version,
      status: mode.status,
      is_canonical: mode.is_canonical,
    }));
  },
});
...
export const updateModeStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (isRejectingCanonical) {
      ...
      const allVersions = await ctx.db
        .query("modes")
        .withIndex("by_name", (q) => q.eq("name", mode.name))
        .collect();
...
export const getModeWithVersions = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Find all versions of this mode by name
    const allVersions = await ctx.db
      .query("modes")
      .withIndex("by_name", (q) => q.eq("name", mode.name))
      .collect();
