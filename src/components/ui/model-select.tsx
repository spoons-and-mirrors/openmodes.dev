...
export function ModelSelect({
  ...
  const modelsData = useQuery(api.query.getModelsData);
            ...
            Object.entries(providerData.models).forEach(
              ([modelId, modelInfo]: [string, any]) => {
                ...
                allModels.push({
                  ...
                  modelName: modelName,
                  searchText:
                    `${provider} ${modelName} ${modelId}`.toLowerCase(),
                });
              },
            );
          }
  ...
  if (modelsData === undefined) {
    return (
      <Button
        variant="outline"
        role="combobox"
        className="w-full h-10 justify-between bg-background-light border-muted text-white opacity-60 cursor-not-allowed"
        disabled
      >
        Loading models...
  ...
  if (modelsData === null || models.length === 0) {
    return (
      <Button
        variant="outline"
        role="combobox"
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="w-full h-10 justify-between bg-background-light border-muted text-white hover:border-accent hover:bg-background-light"
        >
          {selectedModel ? (
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0 bg-background-light border border-muted">
        <Command className="bg-background-light [&_[data-slot=command-input-wrapper]_svg]:text-brand [&_[data-slot=command-input-wrapper]_svg]:opacity-100">
          <CommandInput
